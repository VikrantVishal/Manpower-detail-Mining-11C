...
export {
  db,
  collection, doc, getDoc, getDocs, setDoc, updateDoc, addDoc, deleteDoc, deleteField,
...
export const STATUS_COLORS = {
  ...
  "In other Shift":"#e7e741",
  "Comp-Off":"#4339d1"
};

// ------------------------------------------------------------
// Employee Active/Inactive status
// Status only controls availability for NEW selections (Mark
// Attendance "Add Employee", OT selection, Leave/Tour "Select
// Employee"). It never hides or deletes historical records —
// pages that render existing attendance/leave records key off
// the record itself, not off the employee's current status.
// ------------------------------------------------------------
export const EMPLOYEE_STATUSES = ["Active", "Inactive"];

export function isEmployeeActive(emp) {
  return (emp?.status || "Active") !== "Inactive";
}
